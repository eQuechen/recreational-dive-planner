...
Then("debería ver el título {string}", (title: string) => {
    cy.title().should("contain", title).wait(2000);
});

Then("debería ver el encabezado {string}", (heading: string) => {
    cy.contains("h1", heading).should("be.visible").wait(2000);
});

Then("debería ver el texto {string}", (text: string) => {
    cy.contains(text).should("be.visible").wait(2000);
});

Then("debería ver el enlace {string}", (linkText: string) => {
    cy.contains("a", linkText).should("be.visible").wait(2000);
});

Then("debería ver el botón {string}", (buttonText: string) => {
    cy.contains("button", buttonText).should("be.visible").wait(2000);
});

When("hace clic en el enlace {string}", (linkText: string) => {
    cy.contains("a", linkText).should("be.visible").click().wait(3000);
});

When("el usuario hace click en {string}", (text: string) => {
    cy.contains("button", text).should("be.visible").click().wait(3000);
});

Then("la URL debería contener {string}", (path: string) => {
    cy.url().should("include", path).wait(2000);
});

Then("debería mostrarse el modal de la tabla RDP", () => {
    cy.contains("Tabla RDP (Métrica)").should("be.visible").wait(3000);
});
